...
import { Clock, MessageSquare, Share2, Bookmark, ThumbsUp, ThumbsDown, Crown } from 'lucide-react';
...
const NewsCard: React.FC<NewsCardProps> = ({ article, index = 0 }) => {
  ...
  const handleBookmark = (e: React.MouseEvent) => {
    ...
    toast({
      ...
      description: isBookmarked ? "The article has been removed from your saved list" : "You can find this article in your saved list",
      duration: 2000,
    });
  };
        ...
        <div className="relative overflow-hidden aspect-[16/9]">
          ...
          {article.trending && (
            <div className="absolute top-3 left-3">
              <Badge variant="default" className="bg-genz-orange animate-pulse px-3">
                Trending 🔥
              </Badge>
            </div>
          )}
          ...
          {isHovered && (
            ...
            >
              <motion.button
                className={`p-3 rounded-full ${reaction === 'crown' ? 'bg-primary' : 'bg-white/20'}`}
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                onClick={handleCrown}
              >
                <Crown className={`h-6 w-6 ${reaction === 'crown' ? 'text-white' : 'text-white'}`} />
              </motion.button>
              
              <motion.button
                className={`p-3 rounded-full ${reaction === 'cancel' ? 'bg-destructive' : 'bg-white/20'}`}
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.9 }}
                onClick={handleCancel}
              >
                <ThumbsDown className={`h-6 w-6 ${reaction === 'cancel' ? 'text-white' : 'text-white'}`} />
              </motion.button>
            </motion.div>
          )}
          ...
          <div className="absolute top-3 right-3">
            <motion.button
              className={`p-2 rounded-full ${isBookmarked ? 'bg-primary text-white' : 'bg-white/20 text-white'}`}
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
        ...
        <CardFooter className="p-4 pt-0 border-t flex justify-between items-center">
            ...
            {Object.entries(article.reactions).map(([type, count], idx) => (
              <TooltipProvider key={type}>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>{count} people reacted with {type}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button className="text-muted-foreground hover:text-primary transition-colors" onClick={handleShare}>
