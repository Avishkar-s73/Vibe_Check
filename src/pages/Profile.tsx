...
const Profile = () => {
  ...
  const userData = {
    name: isCurrentUser ? 'Your Profile' : 'Alex Rivera',
    username: isCurrentUser ? 'vibechecker' : 'alexrivera',
    avatar: isCurrentUser ? 'https://i.pravatar.cc/150?img=8' : 'https://i.pravatar.cc/150?img=1',
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <Tabs defaultValue="activity" className="w-full">
              <TabsList className="mb-6">
                ...
                <TabsTrigger value="crowned" className="data-[state=active]:bg-primary data-[state=active]:text-white">
                  ...
                  {userArticles.slice(0, 4).map((article) => (
                    <div key={article.id} className="border rounded-lg p-4 hover:bg-muted/50 transition-colors">
                      <div className="flex justify-between mb-2">
                        <span className="text-sm text-muted-foreground">On article: <strong>{article.title}</strong></span>
                        <span className="text-xs text-muted-foreground">{new Date(article.publishDate).toLocaleDateString()}</span>
                      </div>
                      <p className="text-sm mb-2">"This article is straight facts! No cap, it's the most based take I've seen all week."</p>
